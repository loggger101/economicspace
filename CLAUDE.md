...
compute, zero failures. **These are the current numbers for the whole model and
they supersede every cell below**, including the cislunar 2×2 that follows.

Best cost/revenue, lower is better, 1.0 is breakeven:

| destination | raw, N = 1 | raw, searched | benef, N = 1 | **benef + searched** (the default) |
|---|---|---|---|---|
...
† **Not an optimum — see the saturation warning below.**
...
never changes the evaluable set**, at any destination — as it must not, since N
enters nothing in the mass cascade.

✅  **`cislunar` is still the best case and the margin is wider than the record
showed** — 13.1443× against `lunar_surface`'s 22.5790×, a factor of **1.72** on
the default cell. The ordering `cislunar` < `lunar_surface` < `leo` <
`mars_surface` << `earth_surface` reproduces the v1.14.0 raw ordering exactly
and now holds on **all four** settings rather than one.
...
### 🚨 THE FIRST FULL-CATALOG RUNTIME ON THE `1.17.x` LINE

**No full-catalog run had been made since `1.16.0`**, so the five
performance-only releases had only ever been measured on 150–400-row cells —
exactly what THE SAMPLING RULE says not to extrapolate from. Measured at
`cislunar`, both builds against the same catalog and Stage 2 pass:
...
`1.17.1`, `1.17.2` and `1.17.5` aimed at (the programme ladder) on top of
...
between two settings — the half of THE SAMPLING RULE `1.15.0` added. All are
now full-catalog measurements at `cislunar`:
...
**A default cislunar run is now ~1.6 h, not 6.8 h.** ⚠️  These are `cislunar`
figures; `leo`, `mars_surface` and `earth_surface` cost 2–3× more per cell
(their default cells run 15,316 / 12,186 / 13,581 s).

Full wall clock, all twenty cells, 12 workers:
...
‡ overlapped an analysis process and are a few percent high; the rest are clean.

### ✅ `1.17.7`'s memory bound, MEASURED rather than projected

`1.17.7` bounded `_CALENDAR_CACHE` against a **projected** 11–18 GB on a
full-catalog default cell — a projection nobody could exercise, because no
full-catalog run had been made since `1.16.0` and a 400-row cell shows 18,000
cache entries rather than 70 million. Sampled every 20 s across the campaign,
**the bound holds**, and peak RSS tracks **output size**, not ladder traffic:

| cell | evaluable | peak python RSS |
|---|---|---|
| `lunar_surface` benef (both) | 606,304 | 8.2 GB |
| `cislunar` (all four) | 650–660 k | 8.4–9.0 GB |
| `leo` raw searched | 776,755 | 9.4 GB |
| `leo` benef, N = 1 | 882,429 | **10.4 GB** |

Peak system use 30.4 GB of 68.6 GB. The rise within a cell is the parent
accumulating result rows, not the cache growing.

⚠️  **Two apparent peaks of 11.54 and 10.74 GB are measurement contamination,
not the pipeline** — they are exactly the two cells that overlapped an
`analyse.py` process loading four 650 k-row frames. Recorded rather than
quietly dropped, because attributing your own harness to the thing you are
measuring is how a clean result becomes a false alarm.

**THE SAMPLING RULE now has a third quantity under it.** It was written about
...
### THE FULL CISLUNAR 2×2 (measured 2026-08-11 on calc `1.16.0`)
...
workers, through the Streamlit UI against one Stage 1/2/3 pass.

✅  **All four MODEL values were re-measured on calc `1.17.7` on 2026-08-23 and
reproduce EXACTLY** — see the 20-cell matrix at the top of this file, which is
where the current numbers now live. This section is kept because its
population-level detail (never-worse counts, propellant and vehicle shares,
programme structure, the rig's two bounds) was measured here first and all of
it reproduced.

🚨  **Its RUNTIME figures and every cost ratio derived from them are
SUPERSEDED.** They are `1.16.0` measurements and five performance-only releases
have landed since: the search costs **1.71×**, not 2.98×; beneficiation
**4.67×**, not 7.1×; the 2×2 is **7.77×** corner to corner, not 18.8×. The
crossed-out figures are left in the table below so the delta is legible.

| | search OFF (N = 1) | **search ON** | search cost (`1.16.0`, ~~superseded~~) |
|---|---|---|---|
| **raw** | **26.7863×** | **15.4273×** | ~~1,307 s → 3,890 s (2.98×)~~ → **733 s → 1,253 s (1.71×)** |
| **beneficiated** | **20.5895×** | **13.1443×** | ~~9,300 s → 24,587 s (2.64×)~~ → **3,424 s → 5,692 s (1.66×)** |

Evaluable: **650,921** raw (both settings), **660,253** beneficiated.
~~Beneficiation costs 7.1× raw at search OFF and 6.3× at search ON~~ — on
`1.17.7` it costs **4.67×** and **4.54×**; the two axes are still close to
multiplicative, and the whole 2×2 is **7.77×** corner to corner.
...
searched at `earth_surface`. Only the two flags moved in `1.17.0`; the
destination did not.

✅  **That cell IS measured now — `7,869.88×`, on 2026-08-23** (20-cell matrix
at the top). ⚠️  And it is the one cell in the model that must not be read as an
optimum: at `earth_surface` market saturation is numerically inert, so 100% of
rows run to `max_fleet_ships`. **The genuine configure-nothing default run
returns a ladder-top value, not a located optimum.** See "`earth_surface`'s
SEARCHED CELLS ARE NOT OPTIMA".
...
🚨  ⚠️  *(That 2.98× is a `1.16.0` runtime and is superseded — the search
costs **1.71×** on `1.17.7`. The point the paragraph makes about a SAMPLE
mispredicting a full-catalog ratio still stands, and is why it is kept.)*
...
> 🚨  **RETIRED 2026-08-24 — one DOES win, at `mars_surface` raw with the
> programme search on**, where 2014 YN (M) takes rank 1 on FEEP at 41.8068×,
> 13.4% clear of the runner-up. The four cislunar cells in this table are still
> correct and still reproduce; what is retired is the general claim. See
> "RETIRED — a `replicated`-scaling device DOES win" at the top of this file.
...
> **first measurements, not deltas**. ✅  **The four non-cislunar BENEFICIATED
> cells were measured on 2026-08-24** (20-cell matrix at the top): 35.8051× /
> 48.2714× / 55.3403× / 25,839.48×. They took 2.0–2.5 h each on `1.17.7`, not
> the ~10–20 h estimated here. ⚠️  They must still not be filled in
...
> 🚨  **Superseded 2026-08-24.** Every raw N = 1 cell here was re-measured on
> calc `1.17.7` and the `cislunar`, `lunar_surface` and `mars_surface` figures
> reproduce EXACTLY; `leo` moves −0.004% and `earth_surface` −1.75%, both live
> metal prices rather than the model. The four "*not measured*" beneficiated
> cells are **all measured now**, along with every searched cell. Quote the
> 20-cell matrix at the top of this file. This section is kept for its
> per-destination propellant, vehicle, ISRU and RTG share tables, which the
> re-measurement reproduces.
...
> 🚨  **AND IT DID FLIP — 2026-08-24, at `mars_surface`, on the programme
> search axis.** The paragraph below ends by naming the exact risk; the axis
> that closed the 1.06× was not thruster mass and not the population but
> `optimise_programme_scale`, which this table predates. **`mars_surface` raw
> searched now puts a FEEP mission at rank 1**, 13.4% clear. Read what follows
> as the warning that came true, not as a current claim.
...
The eleven that arrived with a release of their own are listed here rather than
re-explained; each is written up in full under that release's section. The
other ten predate v1.10.0 and are described below. **11 + 10 = 21**, and the
arithmetic is spelled out because it has already rotted once — see below.

⚠️  **THIS HEADING SAID "twenty-two" AND THE PARAGRAPH SAID "ten", AND BOTH
WERE WRONG, IN OPPOSITE DIRECTIONS.** Corrected 2026-08-20. "Twenty-two" dates
from v1.16.0 and counted the *withdrawn* orbital-refuelling charge (12 released
+ 10 older); "ten" dates from v1.17.3 and predates rows this very table had
already gained. So the table listed eleven items under a sentence calling them
ten, beneath a heading counting one that had been removed.

This is precisely the failure "When a number changes, grep the prose too"
exists to catch, and it is worth noticing *where* it happened: not in a
measured cell, but in a **count spelled out in prose** — the exact category
that section names, in the file that names it. A count is a number. Re-derive
it from the table rather than carrying it forward, and if the two disagree,
the table is the one that was checked.
...
⚠️  v1.11.0's **orbital refuelling** charge was a twelfth released item and has
been *withdrawn* — it is the one the old "twenty-two" was still counting, and
it is deliberately NOT in the 21 — it billed a real cost against a scenario this module does not
...
> ✅  **CLOSED 2026-08-24 — the full 20-cell matrix is measured.** Everything
> this section lists as outstanding, except the historical progression, was
> run on calc `1.17.7`: all four non-cislunar beneficiated cells, every
> searched cell at every destination, and a full-catalog runtime for the
> `1.17.x` line. See the 20-cell matrix at the top of this file. The text below
> is kept as the record of what was outstanding at `1.14.0`.
...
> ⚠️  **7.6% IS STALE, and it is stale in the direction that keeps the item
> declined.** Re-measured on v1.17.5: the prologue is **2.3%** of the default
> cell and **2.6%** of raw, so the recoverable ~89% of it is worth about 2%.
> Three later releases (`1.17.1`, `1.17.2`, `1.17.4`) each cut work around it
> without re-measuring it, and this paragraph's own advice is what they were
> not taking. Quote the v1.17.5 figure.
...
- ⚠️  *(2.98× is itself superseded — **1.71×** on `1.17.7`. The sample-vs-full
  lesson is what this bullet is for.)*
...
still rejected for the reasons that release gives (7.6% of the run at the time
— **2.3% as re-measured on v1.17.5** — against splitting a 570-line function
with ~40 locals crossing the seam). What *has*
...
> ✅  **TAKEN IN v1.17.5, and what changed is the SCOPE of the entry, not the
> bar.** Measured alone the block is 3.2%, as recorded above. But the calendar
> multipliers directly below it are a function of the SAME quantity — this
> release had just memoised them separately, on their own global cache — so one
> entry can carry both, and the `_calendar_multipliers_cached` call disappears
> into the lookup instead of surviving it. Together they measure **~3.5% on
> both searched cells**, against a cache that is **opt-in**: `rig_cache=None`
> is this release's code path exactly, which is what `mission_cost_usd` and
> every other caller still get.
>
> ⚠️  The general shape is worth more than the number, and it is one this file
> has now recorded three times in four releases: **an item priced in isolation
> can be under the bar while the same item priced with its neighbour is over
> it.** v1.17.1 declined the prologue split on a mechanism it had not checked;
> v1.14.2's `max` figure was stale by an interpreter version; this one was
> costed without noticing that the adjacent memo shared its key. *Price the
> block, not the line.*
...
## What calc v1.17.5 changed

**Nothing you can measure** — a performance-only stamp; see the stamp table
under "Bump `pipeline_version`". calc `1.17.4 → 1.17.5`, master
`1.20.4 → 1.20.5`; no other module changed, so no Stage 1/2/3 re-run.

Third release in a row aimed at the programme **ladder**, because that is still
where `1.17.0`'s default flip put the work. `1.17.1` cut the ladder's per-call
constants, `1.17.2` stopped it re-running the cost cascade's N-independent
half, and this one stops it re-deriving three more answers that move with far
less than the option does.

🚨  **IT IS THE SMALLEST PERF STAMP IN THE PROJECT, AND THAT IS THE FINDING.**
Six previous perf releases have run through this search; what is left in the
ladder is per-option overhead measured in **tens of nanoseconds**:

| cell | HEAD | `1.17.5` | speed-up |
|---|---|---|---|
| raw, search off | 1.186 s | 1.175 s | **1.01×** |
| beneficiated, search off | 2.260 s | 2.256 s | **1.00×** |
| raw + search | 2.262 s | 2.127 s | **1.06×** |
| **beneficiated + search** (the `1.17.0` **default**) | **4.283 s** | **4.029 s** | **1.06×** |

Interleaved A/B, **both builds imported into one process** and the cells
alternated A,B,A,B…, best of 4 — the construction `1.17.1` had to adopt after a
separate-process measurement reported the default cell as a *slowdown* purely
from host drift. Viable-row counts agree on every cell.

⚠️  **Shaped like `1.17.2`, not like `1.17.4`.** Every item removes work that
only exists when a LADDER exists, so both search-OFF cells are inert and should
be. **Do not quote a single number for it**, and do not expect another 1.5×
from this code path — see "What is left", below.

### One cache entry now carries the whole W-dependent block

The largest of the four, and it is an item **this file had already measured and
declined** — under "What calc v1.17.4 changed", at 3.2%.

`_mission_cost_tail` is called once per programme option, a median of ~42 per
candidate mission. The ladder varies `n_missions` and `missions_per_ship`, and
a block of it moves with **neither**: the rig shares (`mining_rig_cost`,
`rig_gross_share`, `rig_credit_share`, `rig_terminal_value`) are a function of
`missions_sharing_rig` — i.e. of W — and of the prologue. W runs `1 … trips`,
and `trips` is `min(life / stay, max_trips)` with `max_trips` = **5**. So ~42
options were asking for at most ten distinct answers.

**What moved it over the bar was the neighbour, not a re-measurement.** The
calendar multipliers sixty lines below read `missions_sharing_rig`, `cadence`
and `wacc` — the same key — and `1.17.4` had just memoised them *separately*,
on their own global cache. Fold the two into one entry and the
`_calendar_multipliers_cached` call disappears into the lookup rather than
surviving it. Together: **~3.5% on both searched cells.**

> ⚠️  **`1.17.4` costed the line and not the block, and that is now the third
> instance of this shape in four releases.** `1.17.1` declined the prologue
> split on a mechanism it had not checked and was wrong; `1.14.2`'s `max`
> figure was stale by an interpreter version; this one was priced without
> noticing that the adjacent memo shared its key. **Price the block, not the
> line.**

Three things not to "fix":

**The cache is OPT-IN, and that is what keeps the blast radius at one caller.**
`_mission_cost_tail(..., rig_cache=None)` is the `1.17.4` code path exactly —
same branches, same order — and it is what `mission_cost_usd` and every other
caller still get. Only `_price_programme` passes one, and it owns a fresh dict
per candidate, for the same reason `sat_by_fleet` is per-candidate: every other
input to the block lives in `cost_prologue`.

**It is keyed on `missions_sharing_rig`, NOT on `missions_per_ship`.** The two
are the same on the searched path and are not on the other one — a caller that
passes no `missions_per_ship` derives the count from N instead. Keying on the
derived value is correct on both paths, and the `min`/`max` that derives it is
two comparisons, cheap enough to run before the lookup.

**The calendar multipliers MOVED, and the delta that applies them did not.**
Resolving them sixty lines earlier is a move, not a reordering of arithmetic:
they read nothing computed in between. The delta on `total_cost` is still
written on top of the untouched v1.15.0 sum, exactly as `1.16.0` wrote it, so
no term is re-associated. The four-cell hash is what proves it.

### The two O(N) memos were paying for their own keys

`_learning_curve_cached` and `_mining_reliability_cached` are called **455,094
times apiece** on a 150-row beneficiated+searched sample, and both built their
key in Python first: `(int(n), float(rate))` is two conversions and a tuple
allocation before the lookup starts. The key construction **was** the cost, not
the miss — the caches themselves hold one entry per rung of the ladder and hit
essentially always.

`functools.lru_cache(maxsize=None)` hashes the argument tuple in C. Measured on
this machine: **159 → 92 ns** for the two-argument case and **249 → 131 ns**
for the four-argument one.

⚠️  **The `int()` / `float()` normalisation was not doing anything the cache
does not.** Python hashes `1`, `1.0` and `True` to the same slot and compares
them equal, so a caller passing `1.0` where another passed `1` lands on the
same entry either way — which is exactly what the explicit conversion achieved.
Both callers pass an already-normalised `int` regardless: `_mission_cost_tail`
builds `n_missions` as `max(1, int(...))`. Unbounded as the dicts were, and
`maxsize=None` is also `lru_cache`'s fastest path — it skips the eviction
bookkeeping entirely.

### `_objective_key` was parsing a string 457,776 times

```python
if str(config.selection_objective).strip().lower() == "profit":
```

once per rung of every programme ladder, to re-derive one boolean from a config
field that is fixed for the run. 93 ns against a dict lookup's 31 ns. Memoised
in `_selects_on_profit`, keyed on the **raw value** rather than on `id(config)`
— the raw string *is* the input, so two configs naming the same objective share
an entry and a config edited between runs is still answered correctly. The
parse is unchanged; only its repetition is.

⚠️  The unhashable case falls back to parsing and caching nothing, rather than
raising. `selection_objective` is a `str` field and this cannot fire today, but
a `.get()` that can raise inside `selection_key`'s own algebra is not a trade
worth making.

### ISRU feasibility was a per-propellant fact asked per candidate

`isru_feed_kg_per_kg_propellant`'s own docstring calls it "a per-(asteroid ×
propellant) fact". The **propellant** half of it is not even that — and it is
the expensive half. Roughly four rows in five have no ISRU columns and fall
through to the legacy hydrolox name test, which is a `str().strip().lower()`
plus a substring scan, to conclude "no": **48,600 of 62,018 calls** on a
150-row sample took that path.

Split into `_isru_propellant_consts`, derived in `candidate_combos` beside the
pre-filter and sizing tuples and stashed on the row so it crosses the worker
boundary. What is left in the original is the per-body half — the ice fraction
and the recovery — in the same arithmetic and the same order.

⚠️  **Read through a sentinel, not through `.get(key)`.** `None` is a
legitimate answer here — it means "this propellant can never be made from
asteroid material" — so treating an absent key and a cached `None` alike would
re-derive the value on every call for exactly the rows where the answer is no,
i.e. most of them. `_UNSET` distinguishes them. A cache that silently stops
caching is the quiet-wrong-answer shape this repo keeps finding, in its
performance clothing.

### Dead code

`AU_KM` — a named physical constant in the Δv block, unreferenced in all four
modules, `ui.py`, `ui_meta.py` and `build_master.py`. Removed on `1.17.3`'s
precedent. A fresh mechanical scan of every top-level definition, dataclass
field and import across the four modules found **nothing else**, which is
`1.17.3`'s own conclusion holding one release later: the remaining duplication
in this codebase is semantic, not structural.
...
```
raw,          400-row stride  sha16 373aea01ba63a026  MATCH
beneficiated, 150-row stride  sha16 674ec333c59ca991  MATCH
raw + search, 400-row stride  sha16 565e75f4295e7ff9  MATCH
benef+search, 150-row stride  sha16 487d823bf097338d  MATCH
```

Captured **before** the first edit and re-checked after the version bump, so
the hashes above are the same four floats HEAD produced.

**2. The pre-filter agrees with the unpruned search**, which v1.14.1 requires
after touching anything the pre-filter reads — and this release touched
`isru_feed_kg_per_kg_propellant`, which `_prefilter_probe` calls:

```
prune ON vs prune OFF   135/135 identical, sha256 MATCH, 4 of 4 cells
```

**3. Serial and parallel are byte-identical**, required after any change to the
search, and pointed here because this release attaches a THIRD derived tuple to
the propellant rows that get pickled to workers, and adds two module-level
memos that are per-process:

```
raw + search,   1,200-row stride  508 rows | 712684761cac3668 MATCH
benef + search,   400-row stride  158 rows | 1f853b34621cd54e MATCH
```

⚠️  Eight workers came out SLOWER than serial on both rows (22.3 → 31.3 s and
27.4 → 32.4 s). That is v1.10.1's "more workers is not always faster" note on a
small cap — worker startup is ~1.1 s each and both paths pay the 862 MB catalog
load. The sha256 is what these rows are for.

**4. The mass-ledger identity holds exactly**, all four cells:
...
⚠️  Written against `config.mining_hardware_kg`, because the rig is a **config
constant and not an output column** — the identity as this file states it
raises `KeyError` if run verbatim against the CSV. This release's harness hit
that on the first attempt, exactly as the note near the top of the file warns.

**5. Both never-worse invariants hold, and hold exactly:**

```
beneficiated vs raw      pairs 155 | max 1.000000 | worse 0 | declined 21
searched vs unsearched   pairs 155 | max 0.907498 | worse 0 | median +42.5%
```

✅  That **+42.5%** is the committed full-catalog figure (+42.4%) reproduced on
a stride sample four releases later — the one number in this verification that
could have moved without any hash changing.

⚠️  The harness set `delivery_destination` explicitly and kept `MISMATCH` in
its stdout assertion, per "Model assumptions that are load-bearing":
`CALC_CONFIG` defaults to `earth_surface` while the on-disk Stage 2 catalog is
priced for `cislunar`.

### What is left

🚨  **THE RATIO-INDEPENDENT PROLOGUE IS NOW 2.3%, NOT THE 7.6% THIS FILE HAS
BEEN QUOTING SINCE v1.14.2.** Re-measured rather than inherited, by timing the
span from `_evaluate_combo_at_ratio`'s first line to the sizing loop:

| cell | cell time | prologue | share |
|---|---|---|---|
| raw, search off | 1.197 s | 0.031 s over 10,637 calls | **2.6%** |
| benef + search | 4.064 s | 0.094 s over 32,342 calls | **2.3%** |

About 89% of that is redundant (the sweep re-runs it per ratio and per power
source), so the item is worth **~2%** — for splitting a 570-line function with
~40 locals crossing the seam. v1.14.2 declined it at 7.6% and v1.17.4 asserted
it had shrunk; the assertion was right and is now a measurement. **Still
declined, and now on a current number.**

**`integrity_check` is 0.454 s and was deliberately not taken.** It maps all
1,555,667 rows to tuples and factorizes them to recover the ~25 distinct
compositions `_parse_minerals_column` has *already* factorized one function
earlier. The redundancy is real — it is the same finding v1.17.4 made at the
other end of the CSV boundary — but every clean way to close it widens a
contract: `Series.attrs` does not survive assignment into a DataFrame, and
stashing the names in `catalogs` puts a non-DataFrame into a
`Dict[str, pd.DataFrame]`. ⚠️  And the value is upside-down from where it
looks: 0.454 s is ~10% of a 150-row verification cell and **0.03% of a full raw
cislunar pass**. It optimises the harness, not the pipeline.

**Two items are measured, small, and left alone rather than recorded as
unknown:** `_selects_on_profit`'s own call overhead (~0.7%, and inlining it
would put a bare `dict.get` inside the canonical objective function), and
`_ep_stage_kg`'s two `float(cascade.get(...))` reads (~0.5%, and the defaults
are load-bearing for a hand-built cascade).

> ✅  **The first was taken in v1.17.6, and NOT by inlining it.** The objection
> was right — a bare `dict.get` does not belong inside `_objective_key` — and it
> ruled out the wrong fix rather than the item. `_objective_key` grew an
> optional pre-resolved `on_profit`, so the ladder resolves the flag ONCE per
> candidate and hands it in; the branches and both returned floats are
> untouched, so it is still one statement of the rule. **A rejected
> implementation is not a rejected item** — the same shape as v1.17.1 refusing
> the prologue split on a mechanism it had not checked.

**Branch-and-bound on the objective** remains the one big structural item, and
v1.14.1's warning against approximating the bound stands unaltered.

## What calc v1.17.6 changed

**Nothing you can measure** — a performance-only stamp; see the stamp table
under "Bump `pipeline_version`". calc `1.17.5 → 1.17.6`, master
`1.20.5 → 1.20.6`; no other module changed, so no Stage 1/2/3 re-run.

Three releases in a row went at the programme **ladder**, because `1.17.0`'s
default flip put the work there. This one goes at the **PER-ROW WALK** instead
— the part of a run that every one of 1,555,618 catalog rows pays whether or
not it turns out to be evaluable — and that changes the shape of the result.

🚨  **IT IS THE FIRST PERF STAMP IN THIS PROJECT WHOSE RATIO DEPENDS ON THE
ROW CAP, AND THE FIRST WORTH MORE ON RAW THAN ON THE DEFAULT CELL.** Interleaved
A/B, both builds imported into one process and the cells alternated A,B,A,B…,
best of 3, at caps big enough that the fixed cost stops dominating:

| cell | HEAD | `1.17.6` | speed-up | per-row |
|---|---|---|---|---|
...
| **benef + search, 800 rows** (default) | **22.81 / 22.55 s** | **20.51 / 20.45 s** | **1.10-1.11×** | **1.12×** |

⚠️  **At the 150/400-row caps every previous release measured itself on, the
same build reads 1.03-1.10×.** That is not a contradiction and not drift: a
capped run pays a fixed **~1.6 s** — the catalog integrity walk over 1.55 M rows
plus the pre-filter probe — which is half of a 400-row raw cell, all of it
unchanged, and none of it proportional to anything. **Quote the cap with the
ratio.** The per-row column is what a full pass actually pays.

⚠️  Two full interleaved passes are recorded rather than one because the raw
cell moved **1.14× → 1.19×** between them on an otherwise identical build. That
is host drift — the reason `1.17.1` had to adopt the interleaved construction
in the first place — so **treat raw as ~1.15×, not as 1.19×.**

### Composition is a per-TAXONOMY fact, and it was being derived per ROW

The big one, and it is the **third** instance of v1.17.4's finding: *a column
with few distinct values and one Python call per row.* That release found it on
both sides of the CSV boundary — `enrich_composition` writing it, and
`_parse_minerals_column` reading it back. This is the place in between.

`asteroid_bulk_value_usd_per_kg`, `asteroid_phase_table` and
`asteroid_best_phase_usd_per_kg` read exactly five values off the row —
`comp_pgm_enrichment` and the four `FRACTION_TO_MINERAL` fractions — and
nothing else. All five come out of Module 1's `enrich_composition`, which
derives them from `spectral_type` alone: **76 distinct types, ~25 distinct
composition tuples, across 1,555,667 rows** (eleven in a 4,000-row stride).

So `evaluate_asteroid` walked `FRACTION_TO_MINERAL` three times per asteroid —
with a `pd.isna` on a scalar per entry, at ~1 µs each — to re-derive one of a
couple of dozen answers, once for every body in the catalog. Measured on a
4,000-row stride of the real catalog:

| | before | after | |
|---|---|---|---|
| `asteroid_bulk_value_usd_per_kg` | 14.84 µs/row | 1.21 µs | **12.3×** |
| `asteroid_phase_table` | 13.97 µs/row | 1.25 µs | **11.2×** |
| `asteroid_best_phase_usd_per_kg` | 27.66 µs/row | 1.28 µs | **21.6×** |
| *as `evaluate_asteroid` calls them* | *56.5 µs* | *3.74 µs* | |

**~88 s of every full beneficiated pass and ~41 s of a raw one**, paid by every
row whether or not it is evaluable.

⚠️  **That is ~1% of a full pass, not ~7%.** An evaluable row costs ~2.5-24 ms
in the search, so 53 µs is a small share of it — the figure is worth recording
because it is a fixed per-row charge that the *other* items in this release do
not touch, not because it is the whole 1.15×. Do not quote "88 s" as though it
were the release.

✅  **`asteroid_best_phase_usd_per_kg` calls the bulk function itself**, so a
beneficiated run derived the bulk blend twice per asteroid. The memo closes
that as a side effect, which is why it is the 21.6× row.

Three things not to "fix":

**NaN and None must normalise to the SAME key.** All three consumers test
`x is None or pd.isna(x)` and take the identical branch either way, so two rows
differing only in how a missing value is spelled must share an answer. And a
bare `float('nan')` key would never hit — two NaNs are not equal — so the cache
would silently stop caching for exactly the rows that have gaps. That is the
quiet-wrong-answer shape in its performance clothing, and this file already
records it twice: the `"3.0"` merge key that cost NEOWISE four releases, and
`_UNSET` in v1.17.5's `_isru_propellant_consts`.

**Anything that is not a real number or None returns no key at all**, which
sends the caller down the uncached path rather than inventing a key for a value
whose branch behaviour has not been checked. `to_dict("records")` on the
float64 composition columns yields plain floats, so the fast path is what the
pipeline takes; this is the door for a hand-built row.

**`asteroid_phase_table` returns a COPY of the cached list**, deliberately.
Nothing in this module mutates a phase table — checked — but v1.17.4 records
the other half of that argument when it gave every row its own `comp_minerals`
list: handing a million rows one shared mutable object is a trap whether or not
today's code springs it. A copy also keeps the aliasing exactly as it is now,
which matters because `_PHASE_ORDER_CACHE` is keyed on list **identity** and its
single slot was measured against one list per asteroid. ~0.15 µs against the
~14 µs walk it replaces.

### `_infeasible` was a nested def inside the hottest function in the model

`max_return_payload_kg` defined its refusal helper on **every call** — 500,860
of them on a 150-row beneficiated+searched sample, 589,232 on a 2,000-row raw
one. A nested `def` with two defaults measures **~99 ns** against a ~2,105 ns
call: **~5%** of the single most-called function in the mass cascade, for a
function object that is identical every time. Module level now; a fresh dict per
call, exactly as before.

### The programme ladder is a function of `trips` and the config

`programme_options` builds ~42 tuples and `fleet_refinement` its four more, and
both ran **once per surviving candidate** — 10,741 times per 150-row sample —
for a list whose only asteroid-dependent input is `trips`, which is
`min(life / stay, max_trips)` and therefore one of a handful of small integers.
Together **~3.6%** of the default cell. The sorted fleet ladder the refinement
pass needs is derived from the same tuples, so it is cached alongside.

⚠️  `programme_options` itself is **untouched** — it is the single readable
statement of the search's shape and the entry point for anyone outside this
loop. The private `_programme_ladder_cached` only stops the loop asking it the
same question 10,741 times. Keyed on the config **values** the two functions
read rather than on `id(config)`, so a config edited between runs is answered
correctly — the same argument `_selects_on_profit` makes.

### And four smaller readings of the same sentence

Each is "the question is asked at a finer granularity than it has answers",
which is now this codebase's most common defect shape by a wide margin.

- **One per-propellant cache entry in the combo loop, not four lookups.** The
  grid is vehicle-major, so `isru_modes`, both pre-filter constant tuples and
  the identity key were resolved once per (vehicle × propellant) — **714,000
  lookups per 2,000 rows** for 21 propellants' worth of answers.
- **The Δv options resolved to plain scalars once per asteroid.** All three
  call sites re-read them out of the dict and re-ran `float()` / `bool()` /
  `str()` on every (combo × Δv × ISRU) iteration, for a list with two or three
  entries in it.
- **The vehicle's LEO capacity derived once per RUN.**
  `float(vehicle.get("payload_leo_kg", 0) or 0)` was written out in **three**
  places — the combo loop, the pre-filter probe and `_evaluate_combo_at_ratio` —
  and the first ran it 142,800 times per 400 rows for seventeen numbers. It
  joins the fairing volume on `_vehicle_consts`, which `candidate_combos`
  already stashes on the row so it crosses the worker boundary. ⚠️  The
  `math.isfinite(cap) and cap > 0` test is folded in as a third field rather
  than left to each caller: two of the three tested it and one tested only
  `> 0`, and stating it once is what stops them drifting apart.
- **Five reliability ops rows resolved once per run**
  (`_ops_reliability_constants`), which is `_ops_cost_constants` (v1.17.1) and
  `_ops_sizing_constants` (v1.10.1) applied to the one block between them that
  still read `_ops_value` per candidate.

Also: the stage-2 pre-filter built a 9-key dict and splatted it into both of its
solver calls. `**` on that dict measures **558 ns** a call against **146 ns**
for the keywords, plus **306 ns** to build it — ~1.1 µs of a ~16 µs function
that runs once per surviving (vehicle × propellant × Δv × ISRU). Written out
twice instead, same arguments in the same order.

And `_price_programme`'s signature lost its annotations. It is a **nested def**,
so `Optional[int]` — a `typing` subscript — was evaluated every time the
enclosing function ran: **331 ns against 95 ns** for the bare def. The types are
...
```
raw,          400-row stride  sha16 f3dbd86ee6d35fc0  MATCH
beneficiated, 150-row stride  sha16 9bb6c8bb41852b66  MATCH
raw + search, 400-row stride  sha16 3c809fb067c8d034  MATCH
benef+search, 150-row stride  sha16 1d5823f859478c74  MATCH
```

✅  **Those are the four hashes committed for v1.17.4**, so they now reproduce
across two releases rather than one. Re-run after **each** of the seven items
rather than only at the end, which is what localises a break to the change that
caused it.

**2. The pre-filter agrees with the unpruned search** — required by v1.14.1
after touching anything the pre-filter reads, and this release touched the
vehicle capacity it tests against, the Δv scalars it is handed, and stage 2's
call convention:
...
**3. Serial and parallel are byte-identical**, through the rebuilt `master.py`.
This is the check that matters most here: the release adds **four** new
module-level memos (`_COMPOSITION_CACHE`, `_PROGRAMME_LADDER_CACHE`,
`_FLEET_REFINEMENT_CACHE`, `_OPS_RELIABILITY_CACHE`), all per-process, and it
changes what `candidate_combos` attaches to the vehicle dicts that get pickled
to workers.

```
raw + search,   1,200-row stride  508 rows | 2c33eeb22754ec97 MATCH
benef + search,   400-row stride  158 rows | 00ed6eccf1dd6ee7 MATCH
```

✅  Both hashes are the ones v1.17.4 committed for the same two checks.

⚠️  Eight workers came out SLOWER than serial on both rows (20.7 → 29.5 s and
25.1 → 31.4 s). That is v1.10.1's "more workers is not always faster" note on a
small cap. The sha256 is what these rows are for.
...
⚠️  Written against `config.mining_hardware_kg`, because the rig is a **config
constant and not an output column** — the identity as this file states it raises
`KeyError` against the CSV.

**5. Both never-worse invariants hold, and reproduce v1.17.5's figures exactly:**
...
✅  That **+42.5%** is the committed full-catalog **+42.4%** reproduced on a
stride sample five releases later — the one number in this verification that
could have moved without any hash changing.

⚠️  The harness set `delivery_destination` explicitly and asserted `MISMATCH`
was absent from stdout, per "Model assumptions that are load-bearing".

**6. `git status` is clean but for the three expected files** after
`py build_master.py`, and a second rebuild is idempotent.

### What is left

**The fixed ~1.6 s a capped run pays is untouched and was declined again**, for
the reason v1.17.5 gives: `integrity_check`'s walk (~0.9 s here) and the
pre-filter probe (~0.5 s) are ~55% of a 400-row raw cell and **~0.1% of a full
pass**. They optimise the harness this project argues its releases from, not the
pipeline. ⚠️  They are also why **every ratio in this section names its row
cap** — without that, the same build honestly measures 1.03× or 1.19×.

**`_evaluate_combo_at_ratio` is the largest single item again** — its own body,
not its callees — and the ratio-independent prologue inside it is still ~2.3%
and still correctly declined; see v1.17.5's own re-measurement.

**`viability_only` on `max_return_payload_kg` was measured and declined.** The
trailing block plus the 14-key result dict is **519 ns of a 2,105 ns call**
(25%), and the pre-filter's second stage reads only `["viable"]` from its second
call — but that is 31% of the solver's calls in a raw cell and 6% in a
beneficiated one, so the average gain is under 1% against a new branch in the
hottest function in the model and a second shape for its return value. Recorded
rather than taken. ⚠️  Do not "find" it again and assume it is bigger.
