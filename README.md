...
| 16 | borrows | the worked calculation touching something in `master` that is on no row of its own `BORROWED` register, or a row that matches no call. Its Verification footer told every reader that what the derivation reads from the model is "reference data and table accessors", and it was calling the night-side derate, the synodic period, the delivered price and the programme ladder -- the first two inside things that sentence NAMES as written out there. Check 14 cannot see it, because a claim about which functions a file calls carries no digit. The first two are written out now, the price is compared rather than printed, and the footer's sentence is DERIVED from the register by walking the file's AST, so what is still borrowed on purpose is disclosed instead of denied |
