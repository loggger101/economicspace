...
Version is the `pipeline_version` field on CatalogConfig below.

Builds the asteroid catalog that every later stage keys off: fetches from
JPL SBDB, MP3C, SsODNet ssoBFT and NEOWISE, merges and dedupes them,
validates the result, and enriches it with per-spectral-type PGM factors.

Run directly (`python modules/catalog.py`) to build just this stage;
