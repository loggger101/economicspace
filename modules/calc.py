...
re-introduced here):
    ...
    × (1 + WACC)^mission_duration_yr      ← time-value of money, and OFF by
      default since v1.22.0; see apply_wacc_compounding
...
print(f"    Calendar       : "
      + ("NOT charged (model_programme_calendar off - reproduces 1.15.0)"
         if not CONFIG.model_programme_calendar else
         "programme span charged - amortised NRE and rig compound over "
         "T + (W-1)xcadence" if CONFIG.apply_wacc_compounding else
...
print(f"    Cost of capital: "
      + ("up-front lines compounded over the mission, bucketed by when spent"
         if CONFIG.apply_wacc_compounding else
...
print(f"    Reliability    : "
      + ("expected revenue x P(launch) x P(cruise) x P(mining); costs in full"
         if CONFIG.model_reliability else
         "NOT charged - the run answers what it costs IF IT WORKS"))
print(f"    Learning curve : "
      + ("Wright's law at %.0f%% on recurring hardware"
         % (CONFIG.learning_curve_rate * 100.0)
         if CONFIG.model_learning_curve else
         "NOT applied - no serial-production discount"))
print(f"    Market         : {CONFIG.market_model}"
      + ("" if CONFIG.market_model != "capacity_cap" else
         (" - surplus over a ceiling sells at %.0f%% of price"
          % (CONFIG.surplus_price_fraction * 100.0)
          if CONFIG.sell_surplus_at_discount else
