...
run top-to-bottom — the orchestrator at the bottom executes everything.
    ...
    Stage 1  →  Asteroid Catalog        (Module 1, profitability_pipeline 1.0.6)
                JPL SBDB + MP3C + SsODNet + NEOWISE
                (Asterank removed in module v1.0.5)
                + PGM_ENRICHMENT_BY_TYPE per-spectral-type factors
    Stage 2  →  Mineral Value Catalog   (Module 2, MineralValue 1.1.4)
    ...
    Stage 4  →  Profitability Calc      (Module 4, CalcPipeline 1.3.6)
...
# AUTO-RUN
# ─────────────────────────────────────────────────────────────────────────────
# Default invocation — Colab / Jupyter / `python Master(1.4.3).py`.
